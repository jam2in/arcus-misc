...
function kill_exist_g4() {
  echo ">>>>>> $0 running ($COUNTER/$run_count)"
  if [ -f "pidfiles/memcached.127.0.0.1:$g4_m_port" ];
  then
    echo ">>>>>> kill -$kill_type `cat pidfiles/memcached.127.0.0.1:$g4_m_port`"
    kill -$kill_type `cat pidfiles/memcached.127.0.0.1:$g4_m_port`
  fi

  if [ -f "pidfiles/memcached.127.0.0.1:$g4_s_port" ];
  then
    echo ">>>>>> kill -$kill_type `cat pidfiles/memcached.127.0.0.1:$g4_s_port`"
    kill -$kill_type `cat pidfiles/memcached.127.0.0.1:$g4_s_port`
  fi
}
...
do
   ...
   string=`echo cluster join alone | nc localhost $g1_m_port`
   ...
   strung=`echo cluster join alone | nc localhost $g3_m_port`
   check_command_result
   sleep 5
   print_item_count_some
   ...
   ./killandrun.memcached.mg.bash master $g4_m_port NONE
   ./killandrun.memcached.mg.bash slave $g4_s_port NONE
   ...
   string=`echo cluster leave alone | nc localhost $g2_m_port`
   check_command_result
   sleep 1
   kill_exist_g2
   ...
   string=`echo cluster leave alone | nc localhost $g4_m_port`
   check_command_result
   sleep 1
   kill_exist_g3
   ...
   string=`echo cluster leave alone | nc localhost $g1_m_port`
   check_command_result
